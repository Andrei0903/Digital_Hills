...
if(EcwidManager.customer_mail){$scope.customer.customer_mail=EcwidManager.customer_mail
...
EcwidManager.hideLoading()});angular.module("starter").controller('EcwidNewcustomerController',function($scope,$stateParams,$state,$translate,$timeout,Dialog,Ecwid,EcwidManager){$scope.test='newcustomer'});angular.module("starter").factory("Ecwid",function($pwaRequest){var factory={value_id:null,extendedOptions:{}};factory.setValueId=function(valueId){factory.value_id=valueId};factory.loadContent=function(refresh){return $pwaRequest.get('ecwid/mobile_view/load',{urlParams:{value_id:factory.value_id},cache:!refresh})};factory.getCustomer=function(mail){return $pwaRequest.post('ecwid/mobile_view/checkcustomer',{data:{customer_mail:mail},urlParams:{value_id:factory.value_id,},})}
...
service.saveMail=function(){if(typeof localStorage!='undefined'){var mail_json=JSON.stringify(service.customer_mail);localStorage.setItem("woocommerce_customer_mail_"+service.value_id,mail_json)}}
service.getSavedMail=function(){if(localStorage.getItem("woocommerce_customer_mail_"+service.value_id)!=null){var mail_json=localStorage.getItem("woocommerce_customer_mail_"+service.value_id);service.customer_mail=JSON.parse(mail_json)}};service.showLoading=function(){Loader.show()};service.hideLoading=function(){Loader.hide()};if(typeof EcwidManager=="undefined"){service.init()}
